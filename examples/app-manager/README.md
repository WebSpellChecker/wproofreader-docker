...
A successful response confirms that WProofreader Server is reachable and
licensed. A `503` response with `License is absent` means the license ticket
was not accepted; see [Troubleshooting](#troubleshooting). You can also review
the container state with `docker compose ps --all` and its logs with
...
### WProofreader Server rejects requests with "License is absent"

The most common cause is a missing or invalid `LICENSE_TICKET_ID`.

An empty value is caught before anything starts: `docker compose up` stops
with `required variable LICENSE_TICKET_ID is missing a value`.

An invalid value is not caught at startup. WProofreader Server keeps running
without a license and the `appserver` container reports `healthy`, because the
health check only confirms that the process answers. Every API request is
rejected instead:
...
```json
{
  "error": true,
  "is_critical": 0,
  "message": "License is absent. The license activation is required.",
  "command": "status"
}
```

The logs show the activation failure itself:

```bash
docker compose logs appserver | grep -i 'license'
```

```text
Error. The License Ticket ID 'XXXX' that you have entered is incorrect or invalid. Please recheck your Ticket and try again.
License is absent. The license activation is required.
```

Correct `LICENSE_TICKET_ID` in `.env` and recreate the container:

```bash
docker compose up -d appserver
```

Activation contacts the WebSpellChecker license server, so the container needs
outbound internet access. If traffic must go through a proxy, see the proxy
settings in the [main guide](../../README.md).
